...
if __name__ == '__main__':
    ...
    # phs, txt = TxtProcessor.process(t, {'use_tone': False})
    # print(phs, txt)

    pre_align_args = {'use_tone': False, 'forced_align': 'mfa', 'use_sox': True, 'sox_resample': False,
                     'txt_processor': 'zh_g2pM', 'allow_no_txt': False, 'trim_sil': False, 'denoise': False,
                      'with_phsep': True, 'add_eos_bos': True}
    # phs, txt = TxtProcessor.process('他来到了，网易杭研大厦', {'use_tone': True})
    # phs, txt = TxtProcessor.process('我明白太放不开你的爱', pre_align_args)
    phs, txt = TxtProcessor.process('打远一看,它们的确很是美丽,', pre_align_args)
    print(phs)
